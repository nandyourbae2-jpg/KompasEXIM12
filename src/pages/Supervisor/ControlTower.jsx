...
import { Ship, AlertTriangle, Users, Target, Activity, FileText, X, AlertCircle } from 'lucide-react';
...
const ImportSupervisorDashboard = () => {
  ...
  const { pendingApproval: pendingRequests, fetchPending: fetchPendingRequests } = useFinancialRequestStore();
  const { pendingApproval: pendingPibRequests, fetchPending: fetchPendingPibRequests } = usePibRequestStore();
  const navigate = useNavigate();

  React.useEffect(() => {
    fetchPeriodes();
    fetchPendingRequests();
    fetchPendingPibRequests();
  }, [fetchPeriodes, fetchPendingRequests, fetchPendingPibRequests]);

  const mtbPendingCheck = (Array.isArray(periodes) ? periodes : []).filter(p => p.status === 'Submitted' && p.departemen === user?.departemen);
  ...
  const otherTasksList = deptTasks.filter(t => !deptStaff.find(s => s.id === t.assigneeId));
  if (otherTasksList.length > 0) {
    const active = otherTasksList.filter(t => t.status !== 'Selesai');
    const done = otherTasksList.filter(t => t.status === 'Selesai');
    let overdue = 0;
    active.forEach(t => {
      if (t.dueDate) {
        const due = new Date(`${t.dueDate} ${currentYear}`);
    ...
    const completionRate = otherTasksList.length === 0 ? 0 : Math.round((done.length / otherTasksList.length) * 100);

    staffPerformance.push({
      id: 'other',
      name: 'Lainnya / Unassigned',
      total: otherTasksList.length,
      active: active.length,
      done: done.length,
      overdue,
      completionRate,
  ...
  return (
    <div style={{ backgroundColor: 'var(--color-canvas-parchment)', minHeight: '100%', display: 'flex', flexDirection: 'column', fontFamily: 'var(--font-family-body)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '32px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
        ...
        {mtbPendingCheck.length > 0 && (
          <div style={{ backgroundColor: '#FEF2F2', border: '1px solid #FCA5A5', padding: '16px', borderRadius: '8px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
            <AlertCircle size={20} color="#DC2626" style={{ marginTop: '2px' }} />
            <div style={{ flex: 1 }}>
              <h4 style={{ margin: '0 0 4px 0', color: '#991B1B', fontSize: '14px' }}>Approval Realisasi Dana Import (MTB) Menunggu</h4>
              <p style={{ margin: 0, color: '#B91C1C', fontSize: '13px' }}>Ada {mtbPendingCheck.length} buku kas operasional MTB yang memerlukan pengecekan (Checked 1) oleh Anda.</p>
            </div>
            <button 
              onClick={() => navigate('/workspace/realisasi-dana')}
              style={{ backgroundColor: '#DC2626', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '6px', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}
            >
              Cek Sekarang
            </button>
          </div>
        ...
        {pendingRequests.length > 0 && (
          <div style={{ backgroundColor: '#FFFBEB', border: '1px solid #FDE68A', padding: '16px', borderRadius: '8px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
            <AlertTriangle size={20} color="#D97706" style={{ marginTop: '2px' }} />
            <div style={{ flex: 1 }}>
              <h4 style={{ margin: '0 0 4px 0', color: '#92400E', fontSize: '14px' }}>Financial Request Menunggu Pengecekan</h4>
              <p style={{ margin: 0, color: '#B45309', fontSize: '13px' }}>Ada {pendingRequests.length} pengajuan biaya yang memerlukan pengecekan (Checked 1) dari Anda.</p>
            </div>
            <button 
              onClick={() => navigate('/workspace/financial-request')}
              style={{ backgroundColor: '#D97706', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '6px', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}
            >
              Cek Sekarang
            </button>
          </div>
        ...
        {pendingPibRequests.length > 0 && (
          <div style={{ backgroundColor: '#EFF6FF', border: '1px solid #BFDBFE', padding: '16px', borderRadius: '8px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
            <AlertTriangle size={20} color="#2563EB" style={{ marginTop: '2px' }} />
            <div style={{ flex: 1 }}>
              <h4 style={{ margin: '0 0 4px 0', color: '#1E3A8A', fontSize: '14px' }}>PIB Request Menunggu Approval</h4>
              <p style={{ margin: 0, color: '#1D4ED8', fontSize: '13px' }}>Ada {pendingPibRequests.length} permohonan pembayaran PIB yang menunggu approval Anda.</p>
            </div>
            <button 
              onClick={() => navigate('/workspace/pib-request')}
              style={{ backgroundColor: '#2563EB', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '6px', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}
            >
              Review Sekarang
            </button>
          </div>
