...
app.get('/api/tasks', (req, res) => {
  try {
    ...
    tasks.forEach(t => { t.statusHistory = historyStmt.all(t.id); });
    res.json(tasks);
...
app.post('/api/tasks', (req, res) => {
  try {
    const { task_code, judul, deskripsi, departemen, prioritas, status, sumber_tugas, assignee_id, assigned_by_id, import_project_id, tenggat } = req.body;
    db.transaction(() => {
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(task_code, judul, deskripsi, departemen, prioritas, status || 'Backlog', sumber_tugas || 'Manual', assignee_id, assigned_by_id, import_project_id, tenggat);
      const newId = db.prepare('SELECT last_insert_rowid()').pluck().get();
